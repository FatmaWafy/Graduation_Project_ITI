...
// import Cookies from "js-cookie"
// import { AlarmClock } from "lucide-react"
// import StudentMonitor from "../../../../components/monitoring/student-monitor"
...
//     const isViolated = localStorage.getItem(`exam_violated_${examId}`) === "true"

//     if (isViolated) {
//       const reason = localStorage.getItem(`exam_violation_reason_${examId}`) || "Exam rules violation"
//       alert(`You cannot access this exam due to: ${reason}`)
...
//         // Try to fetch from API, use mock data if it fails
//         try {
//           const tempExamRes = await fetch(`http://127.0.0.1:8000/exam/temp-exams/${id}/`, {
//             headers: { Authorization: `Bearer ${token}` },
//           })
...
//           setExam(MOCK_EXAM)
//           setRemainingTime(MOCK_EXAM.duration * 60 || 0)
...
//         setIsSubmitted(stored === "true")
//       } catch (error) {
//         console.error("Error in exam data handling:", error)
...
//       try {
//         const response = await fetch("http://127.0.0.1:8000/exam/exams/submit-exam-answer/", {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
//             Authorization: `Bearer ${token}`,
...
//         setSubmissionStatus(`Exam submitted successfully. Your score: 85%`)
//       }
...
//       <p className="text-xl text-center mb-2 text-gray-700 font-semibold">Duration: {exam.duration} minutes</p>
...
//         {questions.length > 0 ? (
//           questions.map((question) => (
//             <div key={question.id} className="bg-gray-50 p-5 rounded-xl shadow-lg border border-gray-200">
//               <p className="text-xl font-semibold mb-3 text-blue-800">{question.question_text}</p>
//               <div className="space-y-2">
//                 {["option_a", "option_b", "option_c", "option_d"].map((key, index) => (
//                   <label key={index} className="flex items-center gap-2">
...
//           onClick={handleSubmit}
//           disabled={isSubmitted || remainingTime === 0}
//           className="px-6 py-3 bg-gradient-to-r from-blue-700 to-purple-600 text-white font-semibold rounded-full shadow-md hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed"
//         >
//           {isSubmitted ? "Already Submitted" : "Submit Exam"}
//         </button>
//       </div>

//       {submissionStatus && <div className="mt-6 text-center text-green-600 font-bold text-lg">{submissionStatus}</div>}
