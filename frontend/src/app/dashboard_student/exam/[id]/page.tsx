...
const ExamPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchExam = async () => {
      ...
      try {
        ...
        const questionsResponse = await fetch(`http://127.0.0.1:8000/exam/mcq-questions/?exam_id=${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  return (
    <div className="container mx-auto p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-4xl font-bold text-center text-blue-700 mb-4">{exam.title}</h1>
      <p className="text-xl text-center mb-6">Duration: {exam.duration} minutes</p>
      <p className="text-xl text-center mb-6">Time Remaining: {formatTime(remainingTime)}</p>
                ...
                return (
                  <div key={index} className="flex items-center">
                    <input
                      ...
                      className="mr-2"
                    />
                    <label htmlFor={`${question.id}-option-${optionKey}`} className="text-lg">
                      {option}
                    </label>
                  </div>
                );
