...
import { ChevronDown, ChevronUp, Code, FileQuestion, Plus, RefreshCw, Save, Trash2, X } from "lucide-react"
...
export default function AddExamPage() {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const baseUrl =
        questionType === "mcq" ? "http://127.0.0.1:8000/exam/mcq-filter/" : "http://127.0.0.1:8000/exam/coding-filter/"
      ...
      if (selectedLanguage !== "all") {
        ...
        const formattedLanguage = languageMap[selectedLanguage.toLowerCase()] || selectedLanguage
      ...
      setError(`Failed to fetch questions: ${err instanceof Error ? err.message : "Unknown error"}`)
      console.error("Failed to fetch questions:", err)
      toast.error(`Failed to fetch questions: ${err instanceof Error ? err.message : "Unknown error"}`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const addQuestionToExam = (question: Question) => {
    ...
    if (currentType === "mcq") {
      if (!selectedQuestions.some((q) => q.id === question.id)) {
        setSelectedQuestions((prevSelected) => [...prevSelected, questionWithType])
    ...
    try {
      ...
      const newCodingQuestions = questions
        .filter(
          (q) =>
            q.type === "code" &&
            ((q.title && q.title.trim() !== "") || (q.question_text && q.question_text.trim() !== "")),
        )
        .map((q) => ({
      ...
      if (newMCQs.length > 0) {
        for (const mcq of newMCQs) {
          try {
            const mcqResponse = await fetch("http://127.0.0.1:8000/exam/mcq-questions/", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            ...
            if (!mcqResponse.ok) {
              ...
              toast.error(`Failed to create MCQ: ${errorData.message || "Unknown error"}`)
              continue
            }
            ...
            toast.error(`Error creating MCQ: ${error instanceof Error ? error.message : "Unknown error"}`)
          }
        }
      }
      ...
      if (newCodingQuestions.length > 0) {
        for (let i = 0; i < newCodingQuestions.length; i++) {
          ...
          try {
            const codingResponse = await fetch("http://127.0.0.1:8000/exam/code-questions/", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            ...
            if (!codingResponse.ok) {
              ...
              toast.error(`Failed to create coding question: ${errorData.message || "Unknown error"}`)
              continue
            }
            ...
            if (originalQuestion?.test_cases && originalQuestion.test_cases.length > 0) {
              for (const testCase of originalQuestion.test_cases) {
                try {
                  const testCaseResponse = await fetch("http://127.0.0.1:8000/exam/test-cases/", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: `Bearer ${token}`,
                  ...
                  if (!testCaseResponse.ok) {
                    ...
                    toast.error(`Failed to create test case: ${errorData.message || "Unknown error"}`)
                  } else {
                    toast.success("Test case added successfully")
                  }
                } catch (error) {
                  console.error("Error creating test case:", error)
                  toast.error(`Error creating test case: ${error instanceof Error ? error.message : "Unknown error"}`)
                }
              }
            }
          } catch (error) {
            console.error("Error creating coding question:", error)
            toast.error(`Error creating coding question: ${error instanceof Error ? error.message : "Unknown error"}`)
          }
        }
      }
      ...
      const examData = {
        ...
        CodingQuestions: [...codingQuestions.map((q) => q.id), ...createdCodingIds],
      }
  ...
  const getDifficultyBadge = (difficulty: string) => {
    switch (difficulty) {
      case "Easy":
        return <Badge className="bg-green-100 text-green-800 hover:bg-green-200">{difficulty}</Badge>
      case "Medium":
        return <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-200">{difficulty}</Badge>
      case "Hard":
      ...
      <div className="flex flex-col space-y-8">
        <div className="flex flex-col space-y-2">
          ...
          <p className="text-muted-foreground">
            Create and manage your exams with multiple choice and coding questions.
          </p>
        </div>
        ...
        <Tabs defaultValue="question-bank" className="w-full">
          <TabsList className="grid grid-cols-3 mb-6">
            ...
            <TabsTrigger value="selected-questions">
              Selected Questions ({selectedQuestions.length + codingQuestions.length})
            </TabsTrigger>
          ...
          <TabsContent value="question-bank" className="space-y-6">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>Browse and select questions from the existing question bank.</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                  <div className="space-y-2">
                    <Label htmlFor="question-type">Question Type</Label>
                    <Select value={questionType} onValueChange={(value) => setQuestionType(value as "mcq" | "code")}>
                  ...
                  <div className="flex items-end">
                    <Button variant="outline" size="icon" onClick={fetchQuestions} className="h-10 w-10">
                            ...
                            <TableRow key={question.id}>
                              <TableCell className="font-medium">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="line-clamp-1">{question.question_text || question.title}</span>
                                </div>
                              </TableCell>
                              <TableCell>{getDifficultyBadge(question.difficulty)}</TableCell>
                              <TableCell>{question.language || "N/A"}</TableCell>
                                ...
                                <Button
                                  ...
                                  disabled={
                                    questionType === "mcq"
                                      ...
                                      : codingQuestions.some((q) => q.id === question.id)
                                  }
                                >
                                  Add to Exam
                                </Button>
          ...
          <TabsContent value="selected-questions" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Questions that will be included in your exam.</CardDescription>
              </CardHeader>
                  ...
                  <>
                    {selectedQuestions.length > 0 && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium">Multiple Choice Questions ({selectedQuestions.length})</h3>
                    ...
                    {codingQuestions.length > 0 && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium">Coding Questions ({codingQuestions.length})</h3>
                        <div className="rounded-md border divide-y">
                          {codingQuestions.map((question) => (
                            <div key={question.id} className="p-4">
                              <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                  <div className="font-medium">{question.question_text || question.title}</div>
                                ...
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => removeSelectedQuestion(question.id, "code")}
                                  className="text-destructive hover:text-destructive hover:bg-destructive/10"
                                >
                                  <Trash2 className="h-4 w-4" />
                                  <span className="sr-only">Remove</span>
          ...
          <TabsContent value="create-questions" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Create new questions to add to your exam.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <Accordion type="multiple" className="w-full" defaultValue={["question-0"]}>
                  {questions.map((question, index) => (
                    <AccordionItem key={index} value={`question-${index}`} className="border rounded-lg px-2 mb-4">
                      <div className="flex items-center justify-between py-4">
                        <AccordionTrigger className="hover:no-underline">
                          <span className="font-medium">
                            {question.type === "mcq" ? "Multiple Choice Question" : "Coding Question"} {index + 1}
                          </span>
                        </AccordionTrigger>
                        <Button
                          variant="ghost"
                      ...
                      <AccordionContent className="pb-4 space-y-4">
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Select
                              value={question.type}
                              onValueChange={(value) => handleTypeChange(index, value as "mcq" | "code")}
                            >
                              <SelectTrigger>
                          ...
                          <div className="space-y-2">
                            <Label>{question.type === "mcq" ? "Question Text" : "Question Title"}</Label>
                            <Input
                              placeholder={question.type === "mcq" ? "Enter the question" : "Enter the title"}
                              value={question.type === "mcq" ? question.question_text : question.title || ""}
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                handleDifficultyChange(index, value as "Easy" | "Medium" | "Hard")
                              }
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="Select difficulty" />
                          ...
                          {question.type === "mcq" ? (
                            <div className="space-y-4">
                              ...
                              >
                                {["A", "B", "C", "D"].map((option) => (
                                  <div key={option} className="flex items-center space-x-2 space-y-2">
                                    <div className="grid gap-1.5">
                                      <div className="flex items-center space-x-2">
                                        <RadioGroupItem value={option} id={`option-${index}-${option}`} />
                                        <Label htmlFor={`option-${index}-${option}`} className="font-normal">
                                          Option {option}
                                        </Label>
                                      </div>
                                    </div>
                                        ...
                                        <CardContent className="py-2 space-y-4">
                                          <div className="space-y-2">
                                            ...
                                            <Textarea
                                              ...
                                              onChange={(e) =>
                                                handleTestCaseChange(index, tcIndex, "input_data", e.target.value)
                                              }
                                            />
                                          </div>
                                          <div className="space-y-2">
                                            ...
                                            <Textarea
                                              ...
                                              onChange={(e) =>
                                                handleTestCaseChange(index, tcIndex, "expected_output", e.target.value)
                                              }
                                            />
                                          </div>
                                        </CardContent>
        ...
        {debugInfo && (
          <Card>
            ...
            <CardContent>
              <pre className="text-xs font-mono bg-muted p-4 rounded-md overflow-auto max-h-[200px]">{debugInfo}</pre>
            </CardContent>
          </Card>
        )}
      </div>
