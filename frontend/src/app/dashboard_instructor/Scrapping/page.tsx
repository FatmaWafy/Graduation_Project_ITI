...
import { Loader2, Github, Code, ExternalLink, RefreshCw, Filter, Calendar, Download, AlertTriangle } from "lucide-react"
...
export default function ScrappingPage() {
  ...
  const [studentStats, setStudentStats] = useState<Record<number, ExternalStats>>({})
  ...
  const exportToCSV = () => {
    ...
    try {
      const studentsToExport = timeFilter === "all" ? students : filteredStudents
      let csvContent =
      ...
      studentsToExport.forEach((student) => {
        ...
        if (stats?.leetcode_recent_submissions && stats.leetcode_recent_submissions.length > 0) {
          lastActiveDate = format(parseISO(stats.leetcode_recent_submissions[0].timestamp), "yyyy-MM-dd")
        }
      ...
      link.setAttribute("download", `students_${timeFilter}_${format(new Date(), "yyyy-MM-dd")}.csv`)
        ...
        <Card className="border-destructive/50 bg-destructive/10">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <h2 className="text-2xl font-bold text-destructive">Error Loading Students</h2>
              <p className="mt-2 text-muted-foreground">{error.message}</p>
              <Button onClick={() => window.location.reload()} className="mt-6 bg-[#007acc] hover:bg-[#0062a3]">
                Try Again
              </Button>
            </div>
          </CardContent>
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#007acc]">Web Scraping Dashboard</h1>
          <p className="text-muted-foreground">Monitor students' GitHub repositories and LeetCode progress</p>
        </div>
      ...
      {timeFilter !== "all" && filteredStudents.length === 0 ? (
        <Card className="border-[#e6f4ff] bg-white">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-medium text-[#007acc]">No Students Found</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(timeFilter === "all" ? students : filteredStudents).map((student) => (
            <StudentCard
              key={student.id}
              student={student}
...
function StudentCard({ student, stats, isLoading, isRefreshing, onRefresh, onViewDetails }: StudentCardProps) {
  ...
  return (
    <Card className="overflow-hidden border-[#e6f4ff] transition-all duration-200 hover:shadow-md">
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-2 mt-3">
              <CardTitle className="text-xl text-[#007acc]">{username}</CardTitle>
              {isInactive && <AlertTriangle className="h-5 w-5 text-red-500" />}
            </div>
            <CardDescription className="text-center">ID: {student.id}</CardDescription>
          </div>
          <Button
            variant="ghost"
            size="icon"
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-[#f0fff4] text-[#2ea043] font-medium">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-[#fff8f0] text-[#ff9a00] font-medium">
                  {stats?.leetcode_solved !== null ? stats?.leetcode_solved : "N/A"}
                </Badge>
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant="outline"
                className={isInactive ? "bg-red-50 text-red-500" : "bg-[#f0f9ff] text-[#007acc]"}
              >
                {lastActiveDate || "Never"}
              </Badge>
            </div>

            {stats?.leetcode_recent_submissions && stats.leetcode_recent_submissions.length > 0 && (
              <div>
                <div className="text-sm text-muted-foreground mb-2">Recent Submission</div>
                <div className="bg-[#f8fafc] p-2 rounded-md border border-[#e6f4ff]">
                  <div className="font-medium text-sm truncate">{stats.leetcode_recent_submissions[0].title}</div>
                  <div className="text-xs text-muted-foreground">
                    {format(parseISO(stats.leetcode_recent_submissions[0].timestamp), "MMM d, yyyy h:mm a")}
                  </div>
                </div>
              </div>
            )}
      ...
      <CardFooter className="bg-[#f8fafc] border-t border-[#e6f4ff] pt-4">
        <Button onClick={onViewDetails} className="w-full bg-[#007acc] hover:bg-[#0062a3]">
          View Full Details
        </Button>
      </CardFooter>
    </Card>
