...
export function NotificationsDropdown() {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const formattedNotifications = response.data.map(
          (notification: any) => ({
            ...
            is_track_notification:
              notification.is_track_notification || notification.track !== null,
          })
        );
        ...
        const testNotification = {
          message:
            'A new exam "Exam 33" has been scheduled for you at 4/20/2025, 3:07:00 AM.',
  ...
  const hasExamInfo = (message: string) => {
    return (
      ...
      extractExamDateFromMessage(message) !== null
    );
  };
