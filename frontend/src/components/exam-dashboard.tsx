...
// import CodingQuestion from "./coding-question";
// import MultipleChoiceQuestion from "./multiple-choice-question";
// import MultipleChoiceQuestionProps from "./multiple-choice-question";
// import { Button } from "@/components/ui/button";
...
//               // Prepare code with input handling
//               const language = question.language.toLowerCase() as keyof typeof LANGUAGE_VERSIONS;
//               const functionName = testCase.function_name || "solution";
//               const input = testCase.input_data;
...
import CodingQuestion from "./coding-question"
import MultipleChoiceQuestion from "./multiple-choice-question"
import type MultipleChoiceQuestionProps from "./multiple-choice-question"
import { Button } from "@/components/ui/button"
...
export default function ExamDashboard() {
  ...
  // Extract the actual exam ID from the params
  const examId = Array.isArray(id) ? id[0] : id?.toString()

  // Log the exam ID for debugging
  useEffect(() => {
    console.log("ExamDashboard - Exam ID:", examId)
  }, [examId])
  ...
  const [isSubmitted, setIsSubmitted] = useState(false)
  const [timeExpired, setTimeExpired] = useState(false)
  const [codeResults, setCodeResults] = useState<Record<string, TestCaseResult[]>>({})
  const [isLoading, setIsLoading] = useState(false)
  ...
  const handleTestResultsChange = (questionId: string, results: TestCaseResult[]) => {
    setCodeResults((prev) => ({
      ...prev,
      [questionId]: results,
    }))
    ...
    try {
      ...
      for (const question of questions) {
        if (question.type === "coding" && answers[question.id]) {
          try {
            ...
            console.error(`Error running test cases for question ${question.id}:`, error)
            // Create failed test results if execution failed
            updatedCodeResults[question.id] = question.testCases.map(
              (testCase: { input_data: any; expected_output: any }) => ({
                input: testCase.input_data,
      ...
      questions.forEach((question) => {
        ...
        if (question.type === "multiple-choice") {
          ...
          // Calculate points based on test case results
          const results = updatedCodeResults[question.id] || []
          const allPassed = results.length > 0 && results.every((r) => r.isSuccess)
    ...
    <div className="container mx-auto px-4 pt-6 bg-background">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-3 bg-background rounded-lg shadow border border-border">
          {currentQuestion.type === "coding" ? (
            <CodingQuestion
              question={currentQuestion as MultipleChoiceQuestionProps["question"]}
      ...
      <AlertDialog open={showScoreAlert} onOpenChange={setShowScoreAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Exam Completed</AlertDialogTitle>
            <AlertDialogDescription>Your score: {score} Points</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {/* Make sure we're passing the correct exam ID to StudentMonitor */}
      {examId && (
        <div className="mt-4">
          <StudentMonitor examId={examId} />
        </div>
      )}
    </div>
  )
